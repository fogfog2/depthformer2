...
class CMT_Feature(t.nn.Module):
    ...
    def __init__(self, input_width = 640, input_height = 192, embed_dim = 46, start_layer= 2, use_upconv = True, repeats = [2, 2, 2, 2]):
        ...
        Args :
            ...
            --num_classes: default is 1000 for ImageNet
        """
        super(CMT_Feature, self).__init__()

        self.cmt_layer = CMT_Feature_select(
...
class CMT_Feature_select(t.nn.Module):
    """Define CMT model"""
    ...
    def __init__(self,
        ...
        Args :
            ...
            --num_classes: default is 1000 for ImageNet
        """
        super(CMT_Feature_select, self).__init__()
        ...
        #2. Patch Aggregation 1
        self.pa1 = PatchAggregation(in_channels = stem_channels, out_channels = pa_channelses[0], kernel_size=3, stride = 2, padding = 1)
    ...
    def forward(self, x):
        ...
        norm_layer = getattr(self, f'norm0')
        x_out = x_cmtb1.permute(0, 3, 2, 1).contiguous()
        x_out = norm_layer(x_out)
        x_out = x_out.permute(0, 3, 2, 1).contiguous()
        self.features.append(x_out)
