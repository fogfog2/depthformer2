...
########################################################################################################################
    ...
    def _get_next_file(idx, file):
        ...
        if ext == ".png":
            path = os.path.join(os.path.dirname(file), "FrameBuffer_"+str(idx).zfill(4) + ext)
            
        elif ext == ".jpeg":
            path = os.path.join(os.path.dirname(file), "frame"+str(idx).zfill(3) + ext)
        return path
    ...
    def _read_raw_calib_file(folder):
        ...
        return read_calib_file(os.path.join(folder,"colon", CALIB_FILE['cam2cam']))
...
########################################################################################################################
    ...
    def _get_depth_file(image_file, depth_type):
        depth_file = image_file.replace("FrameBuffer","Depth")
        return read_png_depth(depth_file)
    ...
    def _get_sample_context(self, sample_name,
        ...
        forward_context : intforward_context_idxs
        ...
        if parent_folder in self._cache:
            ...
            tt =  os.path.join(parent_folder, '*' + ext)
            kk = glob.glob(tt)
            ff = len(kk)
            max_num_files = len(glob.glob(os.path.join(parent_folder, '*' + ext)))
            self._cache[parent_folder] = (int)(max_num_files)
...
########################################################################################################################
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            ...
            'rgb': load_image(self.paths[idx]).convert('RGB'),
        }
        ...
        sample.update({
            'intrinsics': np.reshape(c_data['P_rect_00'], (3, 4))[:, :3],
        })
        ...
        #if self.with_depth:
        sample.update({
            'depth': self._get_depth_file(self.paths[idx], self.depth_type),
        })
        ...
        if self.with_context:
            ...
            image_context = [load_image(f).convert('RGB') for f in image_context_paths]
            sample.update({
