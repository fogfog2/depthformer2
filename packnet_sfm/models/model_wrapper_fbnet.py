...
class ModelWrapper(torch.nn.Module):
    ...
    @property
    def depth_net_feedback(self):
        """Returns depth network."""
        return self.model.depth_net_feedback
    ...
    def configure_optimizers(self):
        ...
        if self.depth_net is not None:
            ...
            # Depth optimizer
        if self.depth_net_feedback is not None:
            params.append({
                'name': 'Depth_feedback',
                'params': self.depth_net_feedback.parameters(),
                **filter_args(optimizer, self.config.model.optimizer.depth_feedback)
            })
        # Pose optimizer
    ...
    def depth_feedback(self, *args, **kwargs):
        """Runs the pose network and returns the output."""
        assert self.depth_net_feedback is not None, 'Depth network not defined'
        return self.depth_net_feedback(*args, **kwargs)
...
def setup_depth_net_feedback(config, prepared, **kwargs):
    """
    Create a depth network
    ...
    _feedback : nn.Module
        Create depth network
    """
    print0(pcolor('DepthFeedbackNet: %s' % config.name, 'yellow'))
    depth_net_feedback = load_class_args_create(config.name,
        paths=['packnet_sfm.networks.depth_feedback',],
        args={**config, **kwargs},
    )
    if 'CSwin' in config.name:
        if not prepared and config.checkpoint_path is not '':
            depth_net_feedback = load_cswin_network(depth_net_feedback, config.checkpoint_path,
                                    ['depth_net_feedback', 'disp_network'])
    elif 'Swin' in config.name:
        if not prepared and config.checkpoint_path is not '':
            depth_net_feedback = load_swin_network(depth_net_feedback, config.checkpoint_path,
                                    ['depth_net_feedback', 'disp_network'])
    else:
        if not prepared and config.checkpoint_path is not '':
            depth_net_feedback = load_network(depth_net_feedback, config.checkpoint_path,
                                    ['depth_net_feedback', 'disp_network'])
    return depth_net_feedback
...
def setup_model(config, prepared, **kwargs):
    ...
    # Add depth network if required
    if 'depth_net_feedback' in model.network_requirements:
        model.add_depth_net_feedback(setup_depth_net_feedback(config.depth_net_feedback, prepared))
