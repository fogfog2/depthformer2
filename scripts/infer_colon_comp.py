...
def main(args):
    ...
    image_shape = args.image_shape
    if image_shape is None:
        image_shape = config2.datasets.augmentation.image_shape

    # Set debug if requested
    set_debug(config2.debug)
    ...
    if args.input is not None:
        if os.path.isdir(args.input):
            ...
            for idx, image_path in enumerate(files):
                ...
                prev_position, result1= infer_and_vis(image_path,prev_path, args.output, model_wrapper, image_shape, args.half, args.save, 0 ,prev_position)
                prev_position2, result2= infer_and_vis(image_path,prev_path, args.output, model_wrapper2, image_shape, args.half, args.save, 0 ,prev_position)
                
                added = cv2.vconcat([result1,result2])
                cv2.imshow("result", added)
                #cv2.imshow("t2",result2)   
            ...
            while(cap.isOpened()):
                ...
                if ret:
                    if prev_image is not 0:
                        ...
                        prev_position2= infer_and_vis(input_image,prev_image, args.output, model_wrapper2, image_shape, args.half, args.save,  1,prev_position)
